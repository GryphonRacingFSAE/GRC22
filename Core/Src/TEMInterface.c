...
void startTEMInterfaceTask() {
    ...
    while (1) {
    	...
    	bms_broadcast.data[0] = module_number + 1; // The TEM protocol for this number starts from 1, not 0
    	...
    	uint8_t bms_broadcast_checksum = current_thermistor_id + 7;
    	...
    	general_broadcast.data[0] = (uint8_t)(relative_thermistor_id & 0x00FF);
    	general_broadcast.data[1] = (uint8_t)(relative_thermistor_id & 0xFF00);
